...
async def test_capabilities_advertises_session_control_surface(adapter):
    ...
    assert features["session_transcript_revision"] is True
...
@pytest.mark.asyncio
async def test_session_reads_expose_active_transcript_revision_without_archived_rows(
    adapter,
    session_db,
):
    session_id = session_db.create_session("revision-session", "api_server")
    session_db.append_message(session_id, role="user", content="old question")
    session_db.append_message(session_id, role="assistant", content="old answer")
    initial_revision = session_db.get_session(session_id)["transcript_revision"]

    app = _create_session_app(adapter)
    async with TestClient(TestServer(app)) as cli:
        session_resp = await cli.get(f"/api/sessions/{session_id}")
        assert session_resp.status == 200
        session_payload = await session_resp.json()
        assert session_payload["session"]["transcript_revision"] == initial_revision
        ...
        assert messages_payload["transcript_revision"] == initial_revision
        assert [m["content"] for m in messages_payload["data"]] == [
            "old question",
            "old answer",
        ]

        session_db.append_message(session_id, role="user", content="ordinary append")
        append_resp = await cli.get(f"/api/sessions/{session_id}/messages")
        append_payload = await append_resp.json()
        assert append_payload["transcript_revision"] == initial_revision

        session_db.archive_and_compact(
            session_id,
            [{"role": "assistant", "content": "canonical summary"}],
        )
        compacted_resp = await cli.get(f"/api/sessions/{session_id}/messages")
        compacted_payload = await compacted_resp.json()

    assert compacted_payload["transcript_revision"] != initial_revision
    assert [m["content"] for m in compacted_payload["data"]] == [
        "canonical summary"
    ]
    assert len(session_db.get_messages(session_id, include_inactive=True)) == 4


@pytest.mark.asyncio
async def test_empty_session_messages_returns_revision_and_empty_data(
    adapter,
    session_db,
):
    session_id = session_db.create_session("empty-session", "api_server")
    revision = session_db.get_session(session_id)["transcript_revision"]
...
@pytest.mark.asyncio
async def test_session_list_projects_compression_tip_transcript_revision(
    adapter,
    session_db,
):
    session_db.create_session("compression-root", "api_server")
    session_db.append_message("compression-root", "user", "old generation")
    session_db.create_session(
        "compression-tip",
        "api_server",
        parent_session_id="compression-root",
    )
    session_db.append_message("compression-tip", "user", "current generation")
    session_db._conn.execute(
        "UPDATE sessions SET end_reason = 'compression', ended_at = 2.0, "
        "transcript_revision = 'root-revision' WHERE id = 'compression-root'"
    )
    session_db._conn.execute(
        "UPDATE sessions SET started_at = 3.0, transcript_revision = 'tip-revision' "
        "WHERE id = 'compression-tip'"
    )

    app = _create_session_app(adapter)
    async with TestClient(TestServer(app)) as cli:
        resp = await cli.get("/api/sessions?source=api_server")
        assert resp.status == 200
        payload = await resp.json()

    surfaced = next(row for row in payload["data"] if row["id"] == "compression-tip")
    assert surfaced["_lineage_root_id"] == "compression-root"
...
async def test_message_endpoint_reads_revision_and_rows_from_one_sqlite_snapshot(
    ...
    def seed(session_id, revision, content):
    ...
    def rewrite(session_id, content, sampled, committed, errors):
        try:
            assert sampled.wait(5), "reader never reached the ordering handshake"
            session_db.archive_and_compact(
                session_id,
    ...
    # Self-validating negative control: a deliberately split revision/messages
    # read is forced across a committed rewrite and must produce a mixed pair.
    seed("split-control", "split-old", "split old rows")
    split_sampled = threading.Event()
    split_committed = threading.Event()
    split_errors = []
    split_writer = threading.Thread(
        target=rewrite,
        args=(
            "split-control",
            "split new rows",
    ...
    split_writer.start()
    split_revision = session_db.get_session("split-control")["transcript_revision"]
    split_sampled.set()
    assert split_committed.wait(5)
    split_rows = session_db.get_messages("split-control")
    split_writer.join(timeout=5)
    assert not split_writer.is_alive()
    assert split_errors == []
    assert split_revision == "split-old"
    assert [row["content"] for row in split_rows] == ["split new rows"]

    # The real endpoint uses get_active_transcript's single LEFT JOIN statement.
    # Pause immediately after SQLite starts that statement, commit a rewrite,
    # then let fetchall continue: revision and rows must remain the old snapshot.
    seed("atomic-read", "atomic-old", "atomic old rows")
    atomic_sampled = threading.Event()
    atomic_committed = threading.Event()
    atomic_errors = []

    class SnapshotHookConnection(sqlite3.Connection):
        def execute(self, sql, parameters=()):
            cursor = super().execute(sql, parameters)
            if (
                not atomic_sampled.is_set()
                and "transcript_revision AS _transcript_revision" in sql
            ):
                atomic_sampled.set()
                assert atomic_committed.wait(5), "writer did not commit at handshake"
            return cursor
    ...
    read_conn = sqlite3.connect(
        ...
        factory=SnapshotHookConnection,
    )
    read_conn.row_factory = sqlite3.Row

    @contextmanager
    def snapshot_read_ctx():
        yield read_conn

    monkeypatch.setattr(session_db, "_read_ctx", snapshot_read_ctx)
    atomic_writer = threading.Thread(
        target=rewrite,
        args=(
            "atomic-read",
            "atomic new rows",
    ...
    try:
        app = _create_session_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.get("/api/sessions/atomic-read/messages")
            assert resp.status == 200
            payload = await resp.json()
    finally:
        read_conn.close()
        atomic_writer.join(timeout=5)

    assert not atomic_writer.is_alive()
    assert atomic_errors == []
    assert atomic_sampled.is_set(), "real reader did not reach the ordering handshake"
    assert payload["transcript_revision"] == "atomic-old"
    assert [row["content"] for row in payload["data"]] == ["atomic old rows"]
