...
class TestActiveTranscriptRevision:
    def test_existing_sessions_are_backfilled_during_schema_reconciliation(self, tmp_path):
        ...
        conn.execute("DROP TRIGGER sessions_transcript_revision_insert")
        conn.execute("ALTER TABLE sessions DROP COLUMN transcript_revision")
        conn.commit()
        conn.close()

        reopened = SessionDB(db_path=db_path)
        try:
            session = reopened.get_session("legacy")
            assert session is not None
            assert session["transcript_revision"]
        finally:
            reopened.close()

    def test_schema_reconciliation_covers_insert_between_trigger_and_backfill(
        self, tmp_path, monkeypatch
    ):
        db_path = tmp_path / "legacy_interleaving_state.db"
        original = SessionDB(db_path=db_path)
        original.close()

        setup = sqlite3.connect(db_path)
        setup.execute("DROP TRIGGER sessions_transcript_revision_insert")
        setup.execute("ALTER TABLE sessions DROP COLUMN transcript_revision")
        setup.commit()
        setup.close()

        legacy_writer = sqlite3.connect(db_path, isolation_level=None)
        ...
        class InterleavingCursor(sqlite3.Cursor):
            def execute(self, sql, parameters=()):
                ...
                if (
                    not interleaved
                    and "CREATE TRIGGER IF NOT EXISTS sessions_transcript_revision_insert"
                    in sql
                ):
                    interleaved = True
                    legacy_writer.execute(
                        "INSERT INTO sessions (id, source, started_at) "
                        "VALUES ('raced', 'legacy', 1.0)"
                    )
                return super().execute(sql, parameters)

        class InterleavingConnection(sqlite3.Connection):
            def cursor(self, factory=None):
                return super().cursor(factory or InterleavingCursor)

        def connect_with_interleave(*args, **kwargs):
            kwargs["factory"] = InterleavingConnection
            return real_connect(*args, **kwargs)

        monkeypatch.setattr(hermes_state.sqlite3, "connect", connect_with_interleave)
        reopened = SessionDB(db_path=db_path)
        try:
            assert interleaved, "test did not force the migration interleaving"
            raced = reopened.get_session("raced")
            assert raced is not None
            assert raced["transcript_revision"]
        finally:
            reopened.close()
            legacy_writer.close()

    def test_ordinary_append_keeps_revision_stable(self, db):
        db.create_session("s1", source="cli")
        initial = db.get_session("s1")["transcript_revision"]

        db.append_message("s1", role="user", content="hello")
        after_user = db.get_session("s1")["transcript_revision"]
        db.append_message("s1", role="assistant", content="hi")
        after_assistant = db.get_session("s1")["transcript_revision"]

        assert initial
        assert after_user == initial
        assert after_assistant == initial

    def test_batch_append_keeps_revision_stable(self, db):
        db.create_session("s1", source="cli")
        initial = db.get_session("s1")["transcript_revision"]
        ...
        assert inserted == 2
        assert db.get_session("s1")["transcript_revision"] == initial

    def test_compaction_changes_revision_and_hides_archived_rows(self, db):
        db.create_session("s1", source="cli")
        db.append_message("s1", role="user", content="old question")
        db.append_message("s1", role="assistant", content="old answer")
        before = db.get_session("s1")["transcript_revision"]
        ...
        after = db.get_session("s1")["transcript_revision"]
        assert after != before
        assert [m["content"] for m in db.get_messages("s1")] == [
            "canonical summary"
        ]
        assert [m["content"] for m in db.get_messages("s1", include_inactive=True)] == [
            "old question",
            "old answer",
            "canonical summary",
        ]
    ...
    def test_replacement_changes_revision(self, db, active_only):
        db.create_session("s1", source="cli")
        db.append_message("s1", role="user", content="old question")
        before = db.get_session("s1")["transcript_revision"]
        ...
        assert db.get_session("s1")["transcript_revision"] != before
        assert [m["content"] for m in db.get_messages("s1")] == [
            "replacement question"
        ]

    def test_rewind_changes_revision_only_when_active_history_is_removed(self, db):
        db.create_session("s1", source="cli")
        db.append_message("s1", role="user", content="keep question")
        db.append_message("s1", role="assistant", content="keep answer")
        target_id = db.append_message("s1", role="user", content="remove")
        db.append_message("s1", role="assistant", content="remove answer")
        before = db.get_session("s1")["transcript_revision"]
        ...
        repeated = db.rewind_to_message("s1", target_id)
        assert repeated["rewound_count"] == 0
        assert db.get_session("s1")["transcript_revision"] == after

    def test_restoring_rewound_history_changes_revision_only_when_rows_reactivate(self, db):
        db.create_session("s1", source="cli")
        target_id = db.append_message("s1", role="user", content="question")
        db.append_message("s1", role="assistant", content="answer")
        db.rewind_to_message("s1", target_id)
        before_restore = db.get_session("s1")["transcript_revision"]

        assert db.restore_rewound("s1", target_id) == 2
        after_restore = db.get_session("s1")["transcript_revision"]
        assert after_restore != before_restore

        assert db.restore_rewound("s1", target_id) == 0
        assert db.get_session("s1")["transcript_revision"] == after_restore

    def test_clearing_active_history_changes_revision_only_when_rows_are_removed(self, db):
        db.create_session("s1", source="cli")
        db.append_message("s1", role="user", content="question")
        before_clear = db.get_session("s1")["transcript_revision"]

        db.clear_messages("s1")
        after_clear = db.get_session("s1")["transcript_revision"]
        assert after_clear != before_clear
        assert db.get_messages("s1") == []

        db.clear_messages("s1")
        assert db.get_session("s1")["transcript_revision"] == after_clear
