...
class TestAtomicSnapshotWrite:
    ...
    def test_temp_path_is_unique_without_shell_pid_features(self):
        """The temp name must be generated before shell execution.

        macOS still ships bash 3.2, where ``$BASHPID`` is empty.  A shell-side
        suffix therefore collapses concurrent writers back onto one temp file.
        """
        env = _TestableEnv()
        env._snapshot_ready = True
        wrapped = env._wrap_command("echo hi", "/tmp")
        ...
        assert f"{env._snapshot_path}.tmp." in wrapped


    def test_init_session_bootstrap_also_uses_python_unique_temp(self):
        """The init_session bootstrap (first snapshot write) is the same shared
        file a concurrent command could source, so it needs the same
        shell-independent unique temp path."""
        env = _TestableEnv()
        captured = {}
...
class TestAtomicSnapshotConcurrencyBehavioral:
    ...
    that would corrupt PATH.  Each generated writer gets a distinct temp path,
    matching the Python-side UUID generated by ``BaseEnvironment``.
    """
    ...
    def test_concurrent_writes_never_tear_the_snapshot(self, tmp_path):
        ...
        writers = []
        for _ in range(4):
            snap_tmp = _q(f"{snap}.tmp.{uuid.uuid4().hex}")
            writers.append(
