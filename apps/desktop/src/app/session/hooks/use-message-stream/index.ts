  ...
  const completeAssistantMessage = useCallback(
    (
      ...
      const completedState = updateSessionState(sessionId, state => {
        ...
        // ||= not =: an ambiguous non-previewed completion already set
        // shouldHydrate from `existing.interim` above, and a plain assignment
        // here clobbers that decision (#63679).
        shouldHydrate ||=
          !completionError &&
          !hasInlineError &&
